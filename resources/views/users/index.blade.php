                            ...
                            <tbody>
                                @forelse ($users as $user)
                                    <tr class="hover">
                                        ...
                                        <td>
                                            <span class="badge badge-ghost">{{ $user->requisicoes_count ?? $user->requisicoes->count() }}</span>
                                        </td>
                                        <td>
                                            @if ($user->ativo)
                                        ...
                                        <td>
                                            <div class="flex items-center gap-2">
                                                <a href="{{ route('users.edit', $user) }}" class="btn btn-sm btn-outline btn-info" title="Editar">✏️</a>
                                                
                                                {{-- Lógica para não poder inativar a si mesmo --}}
                                                @if ($user->id !== auth()->id())
                                                    <form method="POST" action="{{ route('users.toggle-status', $user) }}" onsubmit="return confirm('Tem a certeza que deseja {{ $user->ativo ? 'inativar' : 'ativar' }} este usuário?');">
                                                        @csrf
                                                        @method('PATCH')
                                                        @if ($user->ativo)
